-- Ciudad/municipio del incidente, para una plataforma de alcance NACIONAL.
--
-- Hasta ahora solo guardábamos 'sector'. La interfaz (SectorsList) asumía que todo
-- sector era de Caracas (o La Guaira), porque no existía un dato estructurado de ciudad.
-- Resultado: un reporte de "La Isabelica, Valencia" se etiquetaba como "Caracas, La
-- Isabelica". Capturar la ciudad la infiere la IA (que ya conoce "Valencia") y permite
-- mostrar "Valencia, La Isabelica" y desambiguar sectores homónimos entre ciudades.
alter table public.incidents
    add column if not exists city varchar;

-- Recrear process_and_deduplicate_incident agregando p_city. Como cambia la firma,
-- eliminamos primero la versión previa de 16 argumentos.
DROP FUNCTION IF EXISTS public.process_and_deduplicate_incident(
    varchar, varchar, varchar, text, varchar, severity_level, text[],
    double precision, double precision, text, varchar, varchar, varchar, varchar, boolean, varchar
);
